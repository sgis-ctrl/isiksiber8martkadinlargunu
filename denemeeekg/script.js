...
let particles = [];
let state = 'INTRO';
// Performance: detect mobile for reduced particle count
const isMobile = window.innerWidth < 700;
const MAX_PARTICLES = isMobile ? 60 : 100;
const PETAL_CHANCE = isMobile ? 0.02 : 0.04;
...
// Performance: throttle canvas draw to 30fps max
let lastFrame = 0;
...
function animate(timestamp = 0) {
    ...
    // Add a few floating petals in BOUQUET state, but cap total particles
    if (state === 'BOUQUET' && particles.length < MAX_PARTICLES) {
    ...
    // Remove particles that have gone off screen in BOUQUET state
    if (state === 'BOUQUET') {
