...
// Performans için partikül limiti (cihaza göre)
const MAX_PARTICLES = window.innerWidth < 600 ? 100 : 200;
...
function finishBouquet() {
    ...
    for (let char of name) {
        ...
        for (let i = 0; i < 3; i++) {
            ...
            `;
            nameFlowersContainer.innerHTML += flowerHTML;
            flowerIndex++;
        }
    }
...
function animate() {
    ...
    // Güncelle, çiz, ve ekran dışına çıkan düşen partikülleri temizle
    particles = particles.filter(p => {
        p.update();
        p.draw();
        // Düşen petal partikülü ekranın altına geçtiyse kaldır
        if (p.isFalling && p.y > height + 20) return false;
        return true;
    });

    if (state === 'BOUQUET') {
        // Petal limitini kontrol ederek yeni petal ekle
        if (particles.length < MAX_PARTICLES && Math.random() < 0.05) {
            ...
            p.opacity = 0.6;
            p.isFalling = true; // Temizlenebilir olarak işaretle
            particles.push(p);
        }
    }
